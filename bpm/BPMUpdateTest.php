<?php
    ...
    $co2 = $_POST['BPMCO2WLTP'];

    $datum = $_POST["BPMproductiedatum"];
    $year_tns = date( 'Y', strtotime( $datum ) );
    $brandstof = $_POST['BPMbrandstof'];


    $years = getYears( $datum );

    if($co2 != '' && $year_tns != '1970' && $co2 != '') {
        foreach($years as $key => $value) {
            // floor( calculateBpmTest( $co2, $value['bpm_date'], $brandstof ) );
            $_POST['BPMArray'][]=floor( calculateBpmTest( $co2, $value['bpm_date'], $brandstof ) );
        }
    }
