 ...
 */
$aColumns = array( 'pl_id', 'pl_pre_order', 'conv1.conversion_name', 'pl_vehicle_id', 'pl_vat_margin', 'cm.cmake_name','cmod.cmodel_name', 'conv.conversion_name', 'cmotor.cmotor_name', 'pl_purchase_price_excl_vat', 'pl_id as edit');
...
while ( $aRow = mysqli_fetch_array( $rResult ) ) {
    ...
    for ( $i = 0; $i < count( $aColumns ); $i ++ ) {
        if ( $aColumns[ $i ] == "conv.conversion_name" ) {
            ...
            $row[] = '<center style="display:flex;"><a href="car_start?car_id='.$aRow[$i].'">'.sprintf("A%'.07d\n", $aRow[$i]).'</a></center>';
        } elseif ( $aColumns[ $i ] != ' ' ) {
            /* General output */
            $row[] = $aRow[$i];
        }
